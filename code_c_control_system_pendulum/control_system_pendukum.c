...
float result = 0;    // результат функции 
//задаем параметры функции синуса(из которой будем получать ошибку) y = A*sin(T*t)
int A = 30 ;//амплитуда , потом будем задавать матлабом 
int T =2 ; // период , потом будем принимать с матлаба 
float error = 0;// для расчета ошибки 
int time12 = 0;
int flag = 0;
